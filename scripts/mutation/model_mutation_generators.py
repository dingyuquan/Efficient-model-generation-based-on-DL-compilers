...
def generate_model_by_model_mutation(model, operator,mutate_ratio=0.3):
    ...
    elif operator == 'ConvReplace':
        mylogger.info("Generating model using {}".format(operator))
        return ConvReplace(model=model)
    else:
...
def all_mutate_ops():
    return ['WS','GF','NEB','NAI','NS','ARem','ARep','LA','LC','LR','LS','MLA','ConvReplace']
...
if __name__ == '__main__':
    ...
    parse.add_argument("--model", type=str, help="model path",default="/home/lemon_proj/IST_LEMON/origin_model/lenet5-mnist_origin.h5")
    parse.add_argument("--mutate_op", type=str, help="model mutation operator", default="LA")
    parse.add_argument("--save_path", type=str, help="model save path",default="/home/lemon_proj/IST_LEMON/lemon_outputs/temp_user1/mutant.h5")
    parse.add_argument("--mutate_ratio", type=float, help="mutate ratio",default=0.1)
