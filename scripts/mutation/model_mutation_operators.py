    ...
    for i, layer in enumerate(layers):
        ...
        if i in mutated_layer_indices:
            ...
            if isinstance(layer.input, list) and len(layer.input) > 1:
                continue
            print('layer input shape {}'.format(layer.input.shape.as_list()))
            print('layer output shape {}'.format(layer.output.shape.as_list()))
...
from tensorflow.keras.layers import Input, Conv2D, ZeroPadding3D,Conv3D
from tensorflow.keras.models import Model


import tensorflow as tf

def ConvReplace(model):
    for layer in model.layers:
        if isinstance(layer, tf.keras.layers.Conv2D):
            ...
            params = {
                ...
                'dilation_rate': layer.dilation_rate + (1,),
            ...
            if kernel_shape[0] != kernel_shape[1]:
                raise ValueError("Kernel must have square shape")
            if kernel_shape[0] % 2 == 0:
                raise ValueError("Kernel size must be odd")
            if not all(kernel_shape[2:] == kernel_shape[2]):
                raise ValueError("Kernel must have equal depth and height")

            kernel = tf.keras.backend.expand_dims(tf.keras.backend.expand_dims(layer.kernel, axis=-2), axis=0)
            x = tf.keras.layers.Conv3D(**params)(tf.keras.layers.Reshape(target_shape=(1,) + layer.input_shape[1:])(layer.input), kernel)
            x = tf.keras.layers.Reshape(target_shape=layer.output_shape[1:-1] + (params['filters'],))(x)
